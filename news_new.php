...
while($businessdetails=mysql_fetch_array($business)){
  $golfbusiness_ids[]=$businessdetails['id'];
  
$ibusinessi++;
} 
if(count($golfbusiness_ids)>0){
$golfbusids=implode(',',$golfbusiness_ids);
// $queryin=" And  `id`  IN (".$busids.")";
}
   
   $newsdata="Select * from  news  where    business_id IN(".$golfbusids.") order by id DESC";
...
$newgolfbusiness=mysql_query("select * from golf_details where id='".$fetchnewsdata['business_id']."'");


$newsgolfbusinessdetails=mysql_fetch_array($newgolfbusiness); 
if($newsgolfbusinessdetails['logo']!=''){
$logoimage=$newsgolfbusinessdetails['logo'];
}else{
$logoimage='null';
}
$business_id=$newsgolfbusinessdetails['id'];
$business_name=$newsgolfbusinessdetails['title_name'];
